...
def affine_layer(x, input_size, output_size, Layer_num):
    W = tf.Variable(tf.random.normal([input_size, output_size], stddev=0.01))
    w = tf.Variable(tf.random.normal([1, output_size], stddev=0.01))
    y = tf.matmul(x, W) + w
    return y
...
HY = tf.compat.v1.placeholder(tf.float32, shape=[None, K])
X = tf.compat.v1.placeholder(tf.float32, shape=[None, K])
HH = tf.compat.v1.placeholder(tf.float32, shape=[None, K, K])
...
learning_rate = tf.compat.v1.train.exponential_decay(startingLearningRate, global_step, decay_step_size, decay_factor,
                                           staircase=True)
